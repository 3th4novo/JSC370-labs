...
```{r}
joint_df <- left_join(met, stations, by = join_by(USAFID == USAF))
```
...
```{r}
quantile(joint_df$temp, na.rm = TRUE)
quantile(joint_df$wind.sp, na.rm = TRUE)
quantile(joint_df$atm.press, na.rm = TRUE)
```
